...
# stale entries), every "# covers:" declaration sits directly above a real
# Scenario/Scenario Outline (an orphaned comment with no scenario beneath it
# credits nothing), and every zero-function module with real top-level
...
Feature: BDD traceability — every Python function sits under a scenario
  ...
  feature files, in both directions. A "# covers:" declaration only counts when
  a real scenario sits directly beneath it, so a comment left behind by a
  deleted scenario cannot keep the gate green with no behavioral coverage.
  ...
  Scenario: Every function in src and scripts is covered by at least one scenario
    ...
    And every coverage declaration sits directly above a real scenario
