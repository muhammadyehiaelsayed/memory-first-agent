...
| T3 | **Memory poisoning** — injected content stored in Redis and replayed as trusted context on future hits (highest-value threat) | **L3 sanitize-before-store** (`security/sanitizer.py`): fetched content is neutralised **once, between markdown conversion and chunking**, so every stored chunk is sanitised text. The per-page **summary** document is LLM-generated from the fetched page, so it is **re-sanitised before storage too** — the summary is not exempt just because a model wrote it — meaning the "stored text is always sanitised text" invariant genuinely holds for every stored document (chunks and summary alike). Injection phrases become the literal marker `[removed-suspicious-instruction]` (never silently deleted). `sanitizer_flags` and a `content_sha256` fingerprint are persisted per document, and the flags are re-attached in the L2 provenance header on every memory hit, so poisoned-but-neutralised content always replays as flagged quoted data. |
...
- **Redis network exposure** is handled as an acknowledged local-only trust boundary rather
  than by in-Redis auth: `docker-compose.yml` binds the published Redis (and RedisInsight)
  ports to `127.0.0.1` only, so no LAN host can reach the unauthenticated instance to read or
  poison memory — consistent with the "auth out of scope" framing for a single-user dev/demo
  stack. Redis `requirepass` / TLS is the production upgrade path if the stack ever leaves
  loopback.
